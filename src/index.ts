...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'MFAPI.in');
}
...
async function searchSchemes(args: Record<string, unknown>): Promise<unknown> {
  ...
  // Token-AND match: every whitespace-separated term must appear somewhere in
  // the scheme name (in any order). Also tries a normalized comparison where
  // non-alphanumeric chars are stripped from both sides — this handles compound
  // words like "Bluechip" matching "Blue Chip" (the actual AMFI name form).
  ...
  const matches = list.filter((s) => {
    ...
    return terms.every((t) => name.includes(t) || normName.includes(strip(t)));
  });
